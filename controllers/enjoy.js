...
var betting = async () => {
	for (let ppp = 0; ppp < bet[0].length; ppp++) {
		...
		if (doc) {
			...
			doc.prices += ttt;
			doc.budget += parseFloat(priceAmount.toFixed(2));
			await doc.save();
		}
	}
	...
	d = new Date();
	d = d.getFullYear() + "" + (1 + parseInt(d.getMonth())) +  new Date().toLocaleString("en-US", {timeZone: "asia"});
	if (old_d && old_d !== d) {
		no = 1;
	}
	old_d = d;
		...
		if (bet[level].length == 0) {
			...
			if (user) {
				...
				return res.status(200).json({
					...
					records_my_page: 1,
					last_records_my_page: Math.ceil(my_enjoy_count / 10),balance:user.budget
				});
			}
			...
			if (status == 0) {
				...
				return res.status(200).json({
					...
					balance:user.budget,balance:user.budget
				});
			} else {
				var _bet = bet[level].find(ele => ele[0] == req.userFromToken._id);
				var _bet_id = bet[level].findIndex(ele => ele[0] == req.userFromToken._id);
...
exports.postEnjoy = async (req, res, next) => {
	try {
		...
		if (status == 0) {
			...
			const bonus1 = parseInt(input_contract)* 0.05;
			const bonus2 = parseInt(input_contract)* 0.03;
